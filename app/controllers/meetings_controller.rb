  ...
  def index
    ...
    if current_user.usertype == "superadmin" 
      if params[:type] == nil
        ...
        @meetings = Meeting.paginate(:conditions=> ["accept =0 and status = 0"] , :page => params[:page], :per_page => 30).order('name')
      elsif params[:type]=='past'#meeting you attended in the past
        #        @meetings = Meeting.find(:all, :conditions=> ["status >= 3"])
        @meetings = Meeting.paginate(:conditions=> ["status >= 3"] , :page => params[:page], :per_page => 30).order('name')
        ...
        @meetings = Meeting.paginate(:conditions => ['status = ? AND paid = ?', 1, false] , :page => params[:page], :per_page => 30).order('name')
      else
        #        @meetings = Meeting.find(:all, :conditions => ['status = ? AND paid = ?', 1, true])
        @meetings = Meeting.paginate(:conditions => ['status = ? AND paid = ?', 1, true], :page => params[:page], :per_page => 30).order('name')
      end
      ...
      if params[:type] == nil
        ...
        @meetings = Meeting.paginate(:conditions=> ["accept =0 and status = 0 and (tutor_id = ? or user_id = ?)", session[:tutor_id], session[:user_id]], :page => params[:page], :per_page => 30).order('name')
      elsif params[:type]=='past'#meeting you attended in the past
        #        @meetings = Meeting.find(:all, :conditions=> ["(tutor_id = ? OR user_id = ? )and status >= ?", session[:tutor_id], session[:user_id], 3])
        @meetings = Meeting.paginate(:conditions=> ["(tutor_id = ? OR user_id = ? )and status >= ?", session[:tutor_id], session[:user_id], 3], :page => params[:page], :per_page => 30).order('name')
      elsif params[:type]=='attending' #meeting list that you will be attending(accepted, paid)
        ...
        @meetings = Meeting.paginate(:conditions => ['(user_id = ? or tutor_id = ?) AND status = ? AND paid = ?', session[:user_id],session[:tutor_id], 1, false], :page => params[:page], :per_page => 30).order('name')
      else
        #        @meetings = Meeting.find(:all, :conditions => ['(user_id = ? or tutor_id = ?) AND status = ? AND paid = ?', session[:user_id],session[:tutor_id], 1, true])
        @meetings = Meeting.paginate(:conditions => ['(user_id = ? or tutor_id = ?) AND status = ? AND paid = ?', session[:user_id],session[:tutor_id], 1, true], :page => params[:page], :per_page => 30).order('name')
      end
